...
from torch_geometric_models import GAT, APPNP2, DAGNN, GCN, GTAN, GTCN
from utils import norm_adj, data_split, clean_A, remove_edge_pts, get_I, separate
...
def test_run(x, y, A, train_mask, val_mask, test_mask, args):
    ...
    for _ in range(args.num_test):
        if args.model == 'GCN':
            ...
            model = APPNP2(args.n_in, args.n_hid, args.n_out, args.dropout, args.dropout2, args.hop, args.alpha)
            g['edge_index'] = A._indices().to(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_test", type=int, default=5, help='num of runs to test accuracy.')
